...
class AcmeTheme {
  ...
  factory AcmeTheme.fromJson(Map<String, dynamic> json) {
    ...
    final customColors = rawThemeData['customColors'] ?? <String, dynamic>{};
    final darkCustomColors =
        rawDarkThemeData['customColors'] ?? <String, dynamic>{};

    List<ThemeExtension> getExtension(Map<String, dynamic> colors) {
      return [
        CustomColors(
          converter: MapCustomColorConverter.new,
          colors: Map<String, dynamic>.from(colors).map(
    ...
    return AcmeTheme(
      ...
      themeData: (decodedLightTheme ?? ThemeData.light()).copyWith(
        extensions: getExtension(customColors),
      ),
      darkThemeData: (decodedDarkTheme ?? ThemeData.dark()).copyWith(
        extensions: getExtension(darkCustomColors),
      ),
